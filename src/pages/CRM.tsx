...
const BOOKING_STAGES = [
  { key: 'qualification', label: 'Qualification', color: '#888' },
  { key: 'magnet_sent', label: 'LM Sent', color: '#7c4dcc' },
  { key: 'follow_up', label: 'Follow Up', color: '#c47a1a' },
  { key: 'booked', label: 'Booked ✓', color: '#1a8c4e' },
...
const CLOSE_STAGES = [
  { key: 'qualification', label: 'Qualification', color: '#888' },
  { key: 'magnet_sent', label: 'LM Sent', color: '#7c4dcc' },
  { key: 'offer_sent', label: 'Offer Sent', color: '#c47a1a' },
  { key: 'discount_sent', label: 'Discount', color: '#dd6b20' },
  { key: 'paid', label: 'Paid ✓', color: '#1a8c4e' },
  { key: 'lost', label: 'Lost', color: '#e53e3e' },
]
...
function getWinField(templateId: string, lead: Lead): boolean {
  if (templateId === 'close-in-chat') return !!lead.paid_at
...
function StatCard({ icon, label, value, sub, color = '#7c4dcc' }: {
  ...
  return (
    <div className="glass" style={{ borderRadius: 16, padding: '18px 20px', flex: 1 }}>
      ...
      <div style={{ fontSize: 26, fontWeight: 800, color: '#111', letterSpacing: '-0.04em', lineHeight: 1 }}>
        {value}
      </div>
...
function FunnelBar({ leads, templateId }: { leads: Lead[]; templateId: string }) {
  const stages = getStages(templateId)
  const total = leads.length || 1
  return (
    <div style={{ display: 'flex', gap: 3, alignItems: 'stretch', height: 6, borderRadius: 10, overflow: 'hidden', marginBottom: 8 }}>
      {stages.map(s => {
        const count = leads.filter(l => l.stage === s.key).length
        const pct = (count / total) * 100
        return pct > 0 ? (
          <div key={s.key} style={{ flex: pct, background: s.color, opacity: 0.7, minWidth: 4, borderRadius: 2 }} title={`${s.label}: ${count}`} />
        ) : null
      })}
    </div>
  )
...
function LeadPanel({ lead, templateId, userId, onClose, onUpdate }: {
  ...
  async function saveNotes() {
    setSavingNotes(true)
    const table = getTable(templateId)
    await supabase.from(table).update({ notes }).eq('id', lead.id)
    onUpdate({ ...lead, notes })
  ...
  if (memory?.memory) {
    try {
      ...
      else if (m.messages) messages = m.messages
      else if (m.chat_history) messages = m.chat_history
    } catch {}
  }
  ...
  return (
    <div style={{
      ...
      <div className="glass-strong" style={{
        ...
        {/* Header */}
        <div style={{
          padding: '20px 22px 18px', borderBottom: '0.5px solid rgba(0,0,0,0.07)',
          display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0,
        }}>
          <div style={{
            width: 44, height: 44, borderRadius: 14,
            background: isWin ? 'rgba(37,211,102,0.12)' : 'rgba(124,77,204,0.09)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 15, fontWeight: 700, color: isWin ? '#1a8c4e' : '#7c4dcc',
            flexShrink: 0,
          }}>
          ...
          <button onClick={onClose} style={{
            ...
            padding: 4, fontSize: 18, display: 'flex', fontFamily: 'inherit',
          }}>
            <i className="ti ti-x" />
          </button>
        </div>
        ...
        <div style={{ padding: '18px 22px', borderBottom: '0.5px solid rgba(0,0,0,0.06)', flexShrink: 0 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#bbb', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 12 }}>
            Journey
          </div>
          <div style={{ display: 'flex', gap: 0 }}>
            {stages.map((s, i) => {
              const idx = stages.findIndex(x => x.key === lead.stage)
              const done = i <= idx
              const current = i === idx
              return (
                <div key={s.key} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
                  <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                    {i > 0 && <div style={{ flex: 1, height: 2, background: done ? s.color : 'rgba(0,0,0,0.08)', opacity: done ? 0.6 : 1 }} />}
                    <div style={{
                      width: 10, height: 10, borderRadius: '50%',
                      background: done ? s.color : 'rgba(0,0,0,0.1)',
                      border: current ? `2px solid ${s.color}` : 'none',
                      flexShrink: 0,
                      boxShadow: current ? `0 0 0 3px ${s.color}22` : 'none',
                    }} />
                    {i < stages.length - 1 && <div style={{ flex: 1, height: 2, background: done && i < idx ? s.color : 'rgba(0,0,0,0.08)', opacity: 0.6 }} />}
                  </div>
                  <span style={{ fontSize: 9, color: current ? s.color : '#bbb', fontWeight: current ? 700 : 400, textAlign: 'center', lineHeight: 1.2 }}>
                    {s.label}
                  </span>
                </div>
              )
              ...
              {messages.slice(-12).map((msg: any, i: number) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    <div style={{
                      maxWidth: '80%', padding: '8px 12px', borderRadius: isHuman ? '12px 12px 3px 12px' : '12px 12px 12px 3px',
        ...
        <div style={{ padding: '16px 22px', flex: 1 }}>
          ...
          <textarea
            ...
            rows={4}
            style={{
              width: '100%', background: 'rgba(255,255,255,0.7)', border: '0.5px solid rgba(0,0,0,0.12)',
          ...
          <button
            ...
            style={{
              marginTop: 8, padding: '7px 16px', borderRadius: 8, fontSize: 12, fontWeight: 600,
              background: notesSaved ? 'rgba(37,211,102,0.09)' : '#111',
              color: notesSaved ? '#1a8c4e' : '#fff',
...
function EmptyState({ hasFlows }: { hasFlows: boolean }) {
  return (
    <div className="glass" style={{
      ...
      <div style={{
        width: 56, height: 56, borderRadius: 18, background: 'rgba(124,77,204,0.08)',
      ...
      <p style={{ fontSize: 13.5, color: '#aaa', lineHeight: 1.65, maxWidth: 320, margin: '0 auto' }}>
        {hasFlows
          ? 'Leads will appear here once your WhatsApp agent starts receiving messages.'
          : 'Deploy a flow first — leads will appear here once your agent is live.'}
      </p>
    </div>
  )
}
...
export default function CRM({ onBack }: Props) {
  ...
  useEffect(() => {
    async function init() {
      ...
      const activeFlows = (data as Flow[]) ?? []
  ...
  const hasActiveFlows = flows.length > 0

  return (
    <div style={{ padding: '48px 40px 80px', maxWidth: 960, margin: '0 auto', fontFamily: 'inherit' }}>
      <style>{`
      ...
      {/* Header */}
      <div style={{ marginBottom: 32, animation: 'crm-up 0.4s ease both' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{
              width: 40, height: 40, borderRadius: 13,
              background: 'rgba(124,77,204,0.09)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
          ...
          {hasActiveFlows && (
            <div style={{ display: 'flex', gap: 6 }}>
              {flows.map(f => (
                <button
                  key={f.id}
                  onClick={() => { setSelectedFlow(f); setStageFilter('all'); setSearch('') }}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 7,
                    padding: '8px 14px', borderRadius: 10, fontSize: 12.5, fontWeight: 600,
                    background: selectedFlow?.id === f.id ? '#111' : 'rgba(0,0,0,0.05)',
                    color: selectedFlow?.id === f.id ? '#fff' : '#666',
      ...
      {!hasActiveFlows ? (
        ...
        <>
          ...
          {leads.length > 0 && selectedFlow && (
            <div className="glass" style={{ borderRadius: 14, padding: '14px 18px', marginBottom: 16, animation: 'crm-up 0.4s ease 0.1s both' }}>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#bbb', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>
                Stage funnel
              </div>
              <FunnelBar leads={leads} templateId={selectedFlow.template_id} />
              <div style={{ display: 'flex', gap: 0 }}>
                {stages.map(s => {
                  const count = leads.filter(l => l.stage === s.key).length
                  return (
                    <div key={s.key} style={{ flex: 1, textAlign: 'center' }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: count > 0 ? s.color : '#ddd' }}>{count}</div>
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 14, animation: 'crm-up 0.4s ease 0.15s both', alignItems: 'center' }}>
            <div style={{ position: 'relative', flex: 1 }}>
              <i className="ti ti-search" style={{
                ...
                fontSize: 14, color: '#ccc', pointerEvents: 'none',
              }} />
              <input
                type="text"
                ...
                style={{
                  ...
                  borderRadius: 10, fontSize: 13, color: '#111', outline: 'none', fontFamily: 'inherit',
                  boxSizing: 'border-box',
                }}
              />
            </div>
            ...
            {leadsLoading ? (
              ...
              filteredLeads.map((lead, i) => (
                <LeadRow
                  key={lead.id}
                  lead={lead}
                  templateId={selectedFlow!.template_id}
                  index={i}
...
function StageFilterBtn({ label, active, color, onClick }: { label: string; active: boolean; color?: string; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{
      ...
      background: active ? (color || '#111') : 'rgba(0,0,0,0.04)',
      color: active ? (color ? '#fff' : '#fff') : '#888',
      border: 'none', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s',
...
function LeadRow({ lead, templateId, index, isSelected, onClick }: {
  lead: Lead; templateId: string; index: number; isSelected: boolean; onClick: () => void
}) {
  const isWin = getWinField(templateId, lead)
  return (
    <div
      ...
      style={{
        ...
        background: isSelected ? 'rgba(124,77,204,0.04)' : 'transparent',
        transition: 'background 0.15s',
        animation: `crm-up 0.3s ease ${index * 0.03}s both`,
        alignItems: 'center',
      }}
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
        <div style={{
          ...
          background: isWin ? 'rgba(37,211,102,0.09)' : 'rgba(124,77,204,0.08)',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 12, fontWeight: 700, color: isWin ? '#1a8c4e' : '#7c4dcc',
        }}>
          {initials(lead.lead_name, lead.phone_number)}
        </div>
      ...
      {/* Source — placeholder until traffic_source is added */}
